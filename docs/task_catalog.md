...
| PPG PRV/perfusion/fiducial proxy | PPG | `PPG_compute_prv`, `PPG_detect_fiducial_points`, `PPG_assess_perfusion_variability` | SDNN/RMSSD/pNN50/LF-HF proxy, onset/notch proxies, amplitude/perfusion flags | PRV is not identical to ECG HRV; onset/notch and perfusion interpretation need calibrated/validated PPG. |
...
| Blood-pressure pulse summary | ABP | `ABP_detect_pulses` | pulse HR, robust systolic/diastolic/MAP/pulse-pressure values | Robust peak-valley detector validated for plausible summaries on 8/8 BIDMC calibrated ABP records; no beat-level reference labels yet. |
