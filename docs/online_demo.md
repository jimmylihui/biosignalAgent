...
Multi-panel plot images are processed panel-by-panel. The color-trace path and segmentation path both return a `signals[]` list, `num_panels`, and per-panel CSV files such as `digitized_signal_panel_01.csv`, while the primary CSV remains the first recovered panel for backward-compatible downstream tool execution. The chat UI renders every recovered panel preview instead of silently showing only the selected/bottom trace.
