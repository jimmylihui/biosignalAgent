...
A first 3-class target-aware U-Net was trained with mask classes `background`, `target_trace`, and `distractor_trace_axes_text_grid`. On the held-out target-aware multi-style validation split it reached mean Dice 0.8168 / IoU 0.6983 for the target trace, compared with 0.8260 / 0.7115 for the current binary multi-style model on the same split. The 3-class model improves some styles such as dark theme and multi-trace, but still struggles on multi-panel + multi-trace. The demo therefore keeps the binary multi-style model as the default for now while retaining the target-aware training path for further data scaling.
