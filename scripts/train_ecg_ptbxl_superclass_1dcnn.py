...
class ECGDataset(Dataset):
    def __init__(self, rows: list[dict], target: str):
        self.rows = rows
        self.target = target
...
class LeadIIResNet(nn.Module):
    def __init__(self, dropout: float = 0.25):
        super().__init__()
        self.stem = nn.Sequential(nn.Conv1d(1, 32, 15, padding=7, bias=False), nn.BatchNorm1d(32), nn.ReLU(inplace=True), nn.MaxPool1d(2))
...
def train_one(rows: list[dict], target: str, model_path: Path, epochs: int, seed: int, batch_size: int, device: str) -> dict:
    seed_all(seed)
    folds = sorted({int(r['strat_fold']) for r in rows})
    y_all = np.asarray([int(r[f'label_{target}']) for r in rows], dtype=int)
    ...
    best_thr = float(max(((f1_score(y_all, oof >= t, zero_division=0), t) for t in np.linspace(0.05, 0.95, 91)), key=lambda z: z[0])[1])
    cv = metrics(y_all, oof, best_thr)

    # Fit final model on all rows for tool use.
    final_ds = ECGDataset(rows, target)
    final_loader = DataLoader(final_ds, batch_size=batch_size, shuffle=True, num_workers=0)
    final_model = LeadIIResNet().to(device)
    pos_weight = torch.tensor([(len(y_all) - y_all.sum()) / max(1, y_all.sum())], dtype=torch.float32, device=device)
    loss_fn = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    ...
    bundle = {'model_state_dict': final_model.cpu().state_dict(), 'target': target, 'threshold': best_thr, 'cv_metrics': cv, 'history': history, 'model': 'LeadIIResNet', 'input_len': 1000, 'label_counts': dict(Counter(map(int, y_all)))}
...
def main() -> None:
    ...
    ap.add_argument('--report-path', type=Path, default=OUT/'ecg_ptbxl_superclass_lead2_resnet_train_report.json')
    ...
    for target in ['cd', 'sttc']:
        report['targets'][target] = train_one(rows, target, args.out_dir / f'ecg_ptbxl_{target}_lead2_resnet.pt', args.epochs, args.seed, args.batch_size, device)
