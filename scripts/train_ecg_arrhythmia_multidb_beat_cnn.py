...
MODEL_PATH = OUT / 'ecg_arrhythmia_multidb_beat_cnn_model.pt'
...
def load_beats_from_specs(specs: list[tuple[str, Path, str, list[str]]], beat_len: int, download: bool) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[str]]:
    X, F, y, groups = [], [], [], []
    for db_name, raw_dir, db_slug, records in specs:
        if download:
            ...
            if missing:
                ...
                files = []
                for r in missing:
                    files += [f'{r}.hea', f'{r}.dat', f'{r}.atr']
                print(f'downloading {db_slug}: {len(missing)} records', flush=True)
                wfdb.dl_files(db_slug, str(raw_dir), files=files, keep_subdirs=False, overwrite=False)
        for record in records:
            ...
            pre = int(0.25 * header.fs)
            post = int(0.45 * header.fs)
            for i, (sample, label) in enumerate(zip(beat_positions, beat_labels)):
                ...
                y.append(int(label))
                groups.append(f'{db_name}:{record}')
            print(f'loaded {db_name}/{record}: {len(beat_positions)} beats', flush=True)
...
def main():
    ...
    parser.add_argument('--mitdb-raw-dir', type=Path, default=Path('/data1/jiahui/biosignal-agent/datasets/raw/mitdb'))
    parser.add_argument('--incart-raw-dir', type=Path, default=Path('/data1/jiahui/biosignal-agent/datasets/raw/incartdb'))
    parser.add_argument('--svdb-raw-dir', type=Path, default=Path('/data1/jiahui/biosignal-agent/datasets/raw/svdb'))
    parser.add_argument('--include', nargs='*', default=['mitdb', 'incartdb'])
    ...
    specs=[]
    if 'mitdb' in args.include:
        specs.append(('mitdb', args.mitdb_raw_dir, 'mitdb', MITDB_RECORDS))
    if 'incartdb' in args.include:
        specs.append(('incartdb', args.incart_raw_dir, 'incartdb', wfdb.get_record_list('incartdb')))
    if 'svdb' in args.include:
        specs.append(('svdb', args.svdb_raw_dir, 'svdb', wfdb.get_record_list('svdb')))
    X,F,y,groups=load_beats_from_specs(specs,args.beat_len,args.download)
    print(json.dumps({'num_beats':int(len(y)),'label_counts':dict(Counter(map(int,y))),'records':len(set(groups)), 'include': args.include}, indent=2))
    ...
    out=OUT/'ecg_arrhythmia_multidb_beat_cnn_train_report.json'; out.write_text(json.dumps(report,indent=2)); print(json.dumps(report,indent=2))
