...
def build_patients(manifest: dict[str, Any], include_unknown: bool = False) -> list[dict[str, Any]]:
    ...
    for rec in manifest['records']:
        ...
        y_outcome = 1 if outcome == 'abnormal' else 0 if outcome == 'normal' else -1
        item = by_patient.setdefault(pid, {'patient_id': pid, 'label': label_name, 'y': y, 'outcome_label': outcome, 'y_outcome': y_outcome, 'records': []})
...
class PatientDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        y_outcome = torch.tensor(int(patient.get('y_outcome', -1)), dtype=torch.long)
        return x, mask, y, y_outcome
...
class PatientMultiLocCNN(nn.Module):
    def __init__(self, emb=128):
        ...
        self.murmur_head = nn.Sequential(nn.Dropout(0.3), nn.Linear(emb, 2))
        self.outcome_head = nn.Sequential(nn.Dropout(0.3), nn.Linear(emb, 2))
    ...
    def forward(self, x, mask):
        ...
        return self.murmur_head(pooled), self.outcome_head(pooled)
...
def run_epoch(model, loader, opt, device, class_weights=None, outcome_weights=None, outcome_loss_weight=0.35):
    model.train()
    murmur_loss_fn=nn.CrossEntropyLoss(weight=class_weights.to(device) if class_weights is not None else None)
    outcome_loss_fn=nn.CrossEntropyLoss(weight=outcome_weights.to(device) if outcome_weights is not None else None)
    total=0.0
    for x,mask,y,y_outcome in loader:
        x=x.to(device); mask=mask.to(device); y=y.to(device); y_outcome=y_outcome.to(device)
        opt.zero_grad(set_to_none=True)
        murmur_logits,outcome_logits=model(x,mask)
        loss=murmur_loss_fn(murmur_logits,y)
        valid=y_outcome>=0
        if bool(valid.any()):
            loss=loss + float(outcome_loss_weight)*outcome_loss_fn(outcome_logits[valid],y_outcome[valid])
...
def predict(model, loader, device):
    ...
    with torch.no_grad():
        for x,mask,y,y_outcome in loader:
            murmur_logits,outcome_logits=model(x.to(device),mask.to(device))
            prob=torch.softmax(murmur_logits,dim=1)[:,1].cpu().numpy().tolist(); ps.extend(prob); ys.extend(y.numpy().tolist())
    return ys,ps
...
def train_fold(patients, train_idx, test_idx, args, seed):
    ...
    y_out=np.asarray([patients[int(i)].get('y_outcome',-1) for i in train_idx],dtype=int); y_out_valid=y_out[y_out>=0]; out_counts=np.bincount(y_out_valid,minlength=2).astype(float) if len(y_out_valid) else np.asarray([1.0,1.0])
    outcome_weights=torch.tensor([len(y_out_valid)/max(1,2*out_counts[0]), len(y_out_valid)/max(1,2*out_counts[1])],dtype=torch.float32)
    ...
    for epoch in range(args.epochs):
        loss=run_epoch(model,train_loader,opt,device,weights,outcome_weights,args.outcome_loss_weight); y,p=predict(model,test_loader,device); m=metrics(y,p,0.5); bt,bm=best_threshold(y,p)
...
def train_full(patients,args):
    ...
    y_out=np.asarray([p.get('y_outcome',-1) for p in patients],dtype=int); y_out_valid=y_out[y_out>=0]; out_counts=np.bincount(y_out_valid,minlength=2).astype(float) if len(y_out_valid) else np.asarray([1.0,1.0])
    outcome_weights=torch.tensor([len(y_out_valid)/max(1,2*out_counts[0]),len(y_out_valid)/max(1,2*out_counts[1])],dtype=torch.float32)
    model=PatientMultiLocCNN(args.embedding_dim).to(device); opt=torch.optim.AdamW(model.parameters(),lr=args.lr,weight_decay=args.weight_decay)
    for _ in range(args.epochs): run_epoch(model,loader,opt,device,weights,outcome_weights,args.outcome_loss_weight)
...
def main():
    ...
    ap.add_argument('--out-model',default='/data1/jiahui/biosignal-agent/outputs/pcg_murmur_patient_multiloc_multitask_cnn.pt')
    ap.add_argument('--report',default='/data1/jiahui/biosignal-agent/outputs/pcg_murmur_patient_multiloc_multitask_cnn_report.json')
    ...
    ap.add_argument('--freq-bins',type=int,default=80); ap.add_argument('--time-bins',type=int,default=128); ap.add_argument('--embedding-dim',type=int,default=128); ap.add_argument('--outcome-loss-weight',type=float,default=0.35); ap.add_argument('--seed',type=int,default=59); ap.add_argument('--cpu',action='store_true')
