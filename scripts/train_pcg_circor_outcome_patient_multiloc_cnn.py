...
def build_patients(manifest: dict[str, Any], include_unknown: bool = False) -> list[dict[str, Any]]:
    ...
    for rec in manifest['records']:
        label = rec.get('outcome') or rec.get('patient_outcome_label') or rec.get('label')
...
def main():
    ...
    ap.add_argument('--manifest',default='/data1/jiahui/biosignal-agent/datasets/processed/circor_pcg_full_murmur_manifest.json')
    ap.add_argument('--out-model',default='/data1/jiahui/biosignal-agent/outputs/pcg_circor_outcome_patient_multiloc_cnn.pt')
    ap.add_argument('--report',default='/data1/jiahui/biosignal-agent/outputs/pcg_circor_outcome_patient_multiloc_cnn_report.json')
    ...
    payload={'model_state_dict':state,'architecture':'PatientMultiLocCNN_logfreq_spectrogram_attention_outcome_abnormal','locations':LOCATIONS,'target_fs':args.target_fs,'seconds':args.seconds,'freq_bins':args.freq_bins,'time_bins':args.time_bins,'embedding_dim':args.embedding_dim,'cv_metrics':cv_metrics,'best_threshold':bt,'best_threshold_metrics':bm,'reference':'CirCor 1.0.3 patient-level outcome abnormal/normal, multi-location attention CNN, stratified patient CV.'}
    Path(args.out_model).parent.mkdir(parents=True,exist_ok=True); torch.save(payload,args.out_model)
    report={'manifest':args.manifest,'num_patients':len(patients),'label_counts':dict(Counter(['abnormal' if v else 'normal' for v in y.tolist()])),'model_out':args.out_model,'folds':folds,'cv_metrics':cv_metrics,'best_threshold':bt,'best_threshold_metrics':bm}
