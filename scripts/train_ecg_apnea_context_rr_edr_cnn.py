...
MODEL_PATH=OUT/'ecg_apnea_rr_edr_context_cnn_model.pt'
...
def load_manifest(path, seq_len, num_channels, context_radius=2):
    m=json.load(open(path)); X=[]; y=[]; groups=[]
    by_record={}
    for r in m['records']:
        by_record.setdefault(str(r['record']), {})[int(r['minute'])]=r
    cache={}
    def load_values(r):
        key=r['path']
        if key not in cache:
            d=load_csv_signal(key,float(r['sampling_rate']),None)
            cache[key]=(d.values,d.sampling_rate)
        return cache[key]
    for r in m['records']:
        rec=str(r['record']); minute=int(r['minute'])
        pieces=[]; fs=float(r['sampling_rate'])
        for offset in range(-context_radius, context_radius+1):
            rr=by_record.get(rec, {}).get(minute+offset)
            if rr is None:
                rr=r
            values, fs=load_values(rr)
            pieces.append(np.asarray(values,dtype=float))
        values=np.concatenate(pieces) if pieces else load_values(r)[0]
        X.append(rr_edr_image(values,fs,seq_len,num_channels)); y.append(1 if r['label']=='apnea' else 0); groups.append(rec)
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--manifest',default='/data1/jiahui/biosignal-agent/datasets/processed/apnea_ecg_large_manifest.json'); ap.add_argument('--seq-len',type=int,default=512); ap.add_argument('--num-channels',type=int,default=6); ap.add_argument('--context-radius',type=int,default=2); ap.add_argument('--epochs',type=int,default=50); ap.add_argument('--seed',type=int,default=41); ap.add_argument('--device',default='cuda' if torch.cuda.is_available() else 'cpu'); ap.add_argument('--model-path',type=Path,default=MODEL_PATH); args=ap.parse_args(); seed_all(args.seed)
    X,y,groups=load_manifest(args.manifest,args.seq_len,args.num_channels,args.context_radius); print(json.dumps({'num':len(y),'labels':dict(Counter(map(int,y))),'groups':len(set(groups)),'shape':list(X.shape),'context_radius':args.context_radius},indent=2))
    ...
    th=float(max(((f1_score(y,proba>=t,zero_division=0),t) for t in np.linspace(0.1,0.9,81)),key=lambda z:z[0])[1]); cv=met(y,proba,th)
    allidx=np.arange(len(y)); final,_=train_fold(X,y,allidx,None,max(args.epochs,45),args.seed+999,args.device)
    args.model_path.parent.mkdir(parents=True,exist_ok=True); torch.save({'state_dict':final.cpu().state_dict(),'architecture':'RREdrCNN_BiLSTM_multichannel','seq_len':args.seq_len,'num_channels':int(X.shape[1]),'context_radius':args.context_radius,'threshold':th,'cv_metrics':cv,'fold_reports':folds,'label_counts':dict(Counter(map(int,y)))},args.model_path)
    report={'model_path':str(args.model_path),'num_rows':int(len(y)),'label_counts':dict(Counter(map(int,y))),'context_radius':args.context_radius,'threshold':th,'cv_metrics':cv,'fold_reports':folds}; (OUT/'ecg_apnea_rr_edr_context_cnn_train_report.json').write_text(json.dumps(report,indent=2)); print(json.dumps(report,indent=2))
