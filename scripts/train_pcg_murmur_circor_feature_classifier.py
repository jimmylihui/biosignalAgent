...
def main():
    ...
    ap.add_argument('--out-model',default='/data1/jiahui/biosignal-agent/outputs/pcg_murmur_circor_feature_classifier.joblib')
    ap.add_argument('--report',default='/data1/jiahui/biosignal-agent/outputs/pcg_murmur_circor_feature_classifier_report.json')
    ...
    summaries={}; best_name=None; best_f1=-1; best_model=None; best_prob=None
    for name,model in models(args.seed).items():
        ...
        if pbm['f1']>best_f1:
            best_f1=pbm['f1']; best_name=name; best_model=model; best_prob=prob
    best_model.fit(x,y)
    payload={'model':best_model,'model_name':best_name,'feature_names':FEATURES,'cv_metrics':summaries[best_name],'reference':'CirCor 1.0.3 location-aware record labels, patient-group CV, patient max aggregation.'}
    Path(args.out_model).parent.mkdir(parents=True,exist_ok=True); joblib.dump(payload,args.out_model)
    report={'manifest':args.manifest,'num_records':len(records),'num_patients':len(set(groups.tolist())),'record_label_counts':dict(Counter(['abnormal' if v else 'normal' for v in y.tolist()])),'best_model':best_name,'best_cv_metrics':summaries[best_name],'all_cv_summaries':summaries,'model_out':args.out_model}
